...
Public Class TWSTravelTimeGarbage
    Private WithEvents GarbageTimer As System.Timers.Timer = New System.Timers.Timer
    ...
    Protected Overrides Sub OnStart(ByVal args() As String)
        Try
            ...
            'ايجاد اونت لوگ براي سرويس براي اعلانات وضعيت سيستم
            MClassConfigurationManagement.CreateTWSGarbageLog()

            'فعال سازي و راه اندازي تايمر 
            GarbageTimer.Interval = TWSClassConfigurationManagement.GetTimerInterval
            GarbageTimer.Enabled = True
            GarbageTimer.Start()

            TWS.ACKSignalRecived(MClassAuthenticationManagement.AuthenticationId, 0, ACKSignal.ServerWinServiceStart, Nothing, "ServerWinServiceStarted", LogSource.ServerWinService, Database.None, "", "TWSTravelTimeGarbage.OnStart", SystemStatus.None, 0)
        Catch ex As Exception
            EventLog.WriteEntry("TWSGarbageSource", ex.Message.ToString, EventLogEntryType.Error)
            TWS.ACKSignalRecived(MClassAuthenticationManagement.AuthenticationId, 0, ACKSignal.AckError, Nothing, ex.Message.ToString, LogSource.ServerWinService, Database.None, "", "TWSTravelTimeGarbage.OnStart", SystemStatus.None, 0)
        End Try
    End Sub
    Protected Overrides Sub OnStop()
        Try
            TWS.ACKSignalRecived(MClassAuthenticationManagement.AuthenticationId, 0, ACKSignal.ServerWinServiceStop, Nothing, "ServerWinServiceStoped", LogSource.ServerWinService, Database.None, "", "TWSTravelTimeGarbage.OnStop", SystemStatus.None, 0)
        Catch ex As Exception
            EventLog.WriteEntry("TWSGarbageSource", ex.Message.ToString, EventLogEntryType.Error)
            TWS.ACKSignalRecived(MClassAuthenticationManagement.AuthenticationId, 0, ACKSignal.AckError, Nothing, ex.Message.ToString, LogSource.ServerWinService, Database.None, "", "TWSTravelTimeGarbage.OnStop", SystemStatus.None, 0)
        End Try
    End Sub
    Private Sub GarbageTimerTick() Handles GarbageTimer.Elapsed
